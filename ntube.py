...
class Class_NTube(object):
	def __init__(self, tube_props, rg0, rl0 ,sampling_rate, C0):
		...
		for i in range(len(tube_props)-1):
			...
			self.r.append((next_tube_props[1]-curr_tube_props[1])/(next_tube_props[1]+curr_tube_props[1]))
	...
	def fone(self, xw):
		...
		for i in range(len(self.tu)):
			for j in range(len(self.tu)-i):
				yb_add = self.r[j+i+1] * self.r[j] * np.exp( -2.0j * np.sum(self.tu[j:j+i+1]) * xw )
	...
	def process(self, yg ):
		...
		for tc0 in range(len(yg)):
			...
			for i in range(len(self.tu)):
				#Handling y1
				if (i == 0):
					y1[i][0] = ((1. + self.r[i] ) / 2.) * yg[tc0] + self.r[i] * y2[i][-1]
				else:
					y1[i][0] = ( 1 + self.r[i] ) * y1[i-1][-1] + self.r[i] * y2[i][-1]


				#Handling y2
				y2[i][0] = -1. * self.r[i+1]  * y1[i][-1]
				if (i != len(self.tu)-1):
					y2[i][0] += ( 1. - self.r[i+1] ) * y2[i+1][-1]
